...
const SubjectTopics = () => {
  ...
  const [filteredPaperSections, setFilteredPaperSections] = useState<PaperSectionType[]>([])
  ...
  const fetchSections = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/paperSection/${paperId}`, {
        withCredentials: true
      })
  ...
  useEffect(() => {
    ...
    const filteredSections = paperSections.filter(
      section => section.name.toLowerCase().includes(search)
    )
      ...
      <div className='sticky top-0 bg-white'>
        <div className='flex flex-row justify-between items-center p-4'>
          ...
          <div className='flex flex-row gap-2'>
            ...
            <AddPaperSection fetchSections={fetchSections} paperId={paperId as string} />
          </div>
        </div>
      </div>

      {subjectTopics.length>0 && <section className='max-h-[75vh] min-w-[80vw] border'>
      <h2 className='font-semibold p-2 bg-gray-50 text-gray-500'>SUBJECT TOPICS</h2>
      <SubjectTopicList topics={filteredSubjectTopics} fetchTopics={fetchTopics} />
      </section>}

      {paperSections.length>0 && <section className='max-h-[75vh] min-w-[80vw] border'>
      <h2 className='font-semibold p-2 bg-gray-50 text-gray-500'>PARTS</h2>
      <PaperSectionList sections={filteredPaperSections} fetchSections={fetchSections} />
      </section>}
...
const PaperSectionList = ({ sections, fetchSections }: { sections: PaperSectionType[], fetchSections: () => void }) => {
      ...
      {sections.map((sub: PaperSectionType, index: number) => (
        <TableRow key={index} className='cursor-pointer'>
          ...
          <TableCell className='flex flex-row gap-2'>
            <Button
              onClick={() => {
                router.push(`/syllabus/subject/paper/paperSection/${sub._id}/${sub.name}`)
              }}
              className='flex items-center gap-2 bg-gradient-to-b from-gray-600 to-gray-900 hover:scale-105 transition-all duration-300'
            >
              View
            ...
            <UpdatePaperSectionDialog fetchSections={fetchSections} paperSection={sub}/>
            <DeletePaperSectionModalButton fetchSections={fetchSections} sectionId={sub._id} sectionName={sub.name}/>
          </TableCell>
        </TableRow>
      ))}
    </TableBody>
...
const AddPaperSection = ({ fetchSections, paperId }: { fetchSections: () => void, paperId: string }) => {
  ...
  const AddPaperSectionHandler = async () => {
    try {
      setLoading(true)
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/paperSection`,
        {
          name: sectionName,
  ...
  return <Dialog>
    ...
    <DialogContent>
      ...
      <div className='flex flex-col gap-2'>
        <Input value={sectionName} onChange={(e) => { setSectionName(e.target.value) }} type="text" placeholder="Paper Section Name"></Input>
        <Textarea value={sectionDesc} onChange={(e) => { setSectionDesc(e.target.value) }} rows={2} placeholder="Paper Section Description (optional)"></Textarea>
      </div>
      <DialogFooter>
        <Button type='submit' onClick={AddPaperSectionHandler}
        className='bg-gradient-to-b from-gray-600 to-gray-900 text-white rounded-md shadow-sm p-2 px-4 text-sm font-semibold hover:scale-105 duration-300 transition-all'
        >
          {
...
const UpdatePaperSectionDialog = ({ fetchSections, paperSection }: { fetchSections: () => void, paperSection: PaperSectionType }) => {
  ...
  const updatePaperSectionHandler = async () => {
    try {
      setLoading(true)
      const response = await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/paperSection/${paperSection._id}`,
        {
          name: sectionName,
          description: sectionDesc,
          paperId: paperId,
  ...
  return <Dialog>
    ...
    <DialogContent>
      ...
      <div className='flex flex-col gap-2'>
        <Input defaultValue={sectionName} onChange={(e) => { setSectionName(e.target.value) }} type="text" placeholder="Subject Name"></Input>
        <Textarea defaultValue={sectionDesc} onChange={(e) => { setSectionDesc(e.target.value) }} rows={2} placeholder="Subject Description (optional)"></Textarea>
      </div>
      <DialogFooter>
        <Button type='submit' disabled={loading} onClick={updatePaperSectionHandler}>
        {
...
const DeletePaperSectionModalButton = ({ sectionId, sectionName, fetchSections }: { sectionId: string, sectionName: string, fetchSections: () => void }) => {
  ...
  const deleteSectionHandler = async () => {
    try {
      setLoading(true)
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/paperSection/${sectionId}`, {
        withCredentials: true
      })
      toast({
