...
const SubTopics = () => {
  ...
  const fetchTopics = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/topics/${subjectTopicId}`, {
      ...
      <div className='sticky top-0 bg-white'>
        <div className='flex flex-row justify-between items-center p-4'>
          ...
          <div className='flex flex-row gap-2'>
            ...
            <AddSubTopic fetchTopics={fetchTopics} subjectTopicId={subjectTopicId as string} />
          </div>
        </div>
      </div>
...
const SubTopicList = ({ topics, fetchTopics }: { topics: SubTopicType[], fetchTopics: () => void }) => {
      ...
      {topics.map((sub: SubTopicType, index: number) => (
        <TableRow key={index} className='cursor-pointer'>
          ...
          <TableCell className='flex flex-row gap-2'>
            <Button
              onClick={() => {
                router.push(`/syllabus/subject/paper/subjectTopic/subTopic/data/${sub._id}/${sub.name}`)
              }}
            ...
            <DeleteSubTopicModalButton topicId={sub._id} topicName={sub.name} fetchTopics={fetchTopics} />
...
const AddSubTopic = ({ fetchTopics, subjectTopicId }: { fetchTopics: () => void, subjectTopicId: string }) => {
  ...
  const addSubTopicHandler = async () => {
    try {
      setLoading(true)
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic`,
        {
          name: subTopicName,
  ...
  return <Dialog>
    ...
    <DialogContent>
      ...
      <div className='flex flex-col gap-2'>
        <Input value={subTopicName} onChange={(e) => { setSubTopicName(e.target.value) }} type="text" placeholder="Sub Topic Name"></Input>
        <Textarea value={subTopicDesc} onChange={(e) => { setSubTopicDesc(e.target.value) }} rows={2} placeholder="Sub Topic Description (optional)"></Textarea>
        ...
        <ToggleGroup type="single" value={type} onValueChange={setYear} className="justify-start">
      ...
      <DialogFooter>
        <Button type='submit' onClick={addSubTopicHandler}
...
const UpdateSubTopicDialog = ({ fetchTopics, subTopic }: { fetchTopics: () => void, subTopic: SubTopicType }) => {
  ...
  const updateSubTopicHandler = async () => {
    try {
      setLoading(true)
      const response = await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/${subTopic._id}`,
        {
          name: subTopicName,
          description: subTopicDesc,
          subjectTopicId: subjectTopicId,
  ...
  return <Dialog>
    ...
    <DialogContent>
      ...
      <div className='flex flex-col gap-2'>
        <Input defaultValue={subTopicName} onChange={(e) => { setSubTopicName(e.target.value) }} type="text" placeholder="Sub Topic Name"></Input>
        <Textarea defaultValue={subTopicDesc} onChange={(e) => { setSubTopicDesc(e.target.value) }} rows={2} placeholder="Sub Topic Description (optional)"></Textarea>
      ...
      <DialogFooter>
        <Button type='submit' disabled={loading} onClick={updateSubTopicHandler}>
        {
...
const DeleteSubTopicModalButton = ({ topicId, topicName, fetchTopics }: { topicId: string, topicName: string, fetchTopics: () => void }) => {
  ...
  const deleteTopicHandler = async (topicId: string, topicName: string) => {
    try {
      setLoading(true)
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/${topicId}`, {
        withCredentials: true
      })
      toast({
