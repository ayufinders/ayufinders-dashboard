      ...
      <div>
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="item-1">
            <AccordionTrigger className="font-bold text-gray-600 text-lg no-underline hover:no-underline">MCQ Questions</AccordionTrigger>
            <AccordionContent className="border">
              <QuestionsList questions={filteredMcqQuestions} />
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-2">
            <AccordionTrigger className="font-bold text-gray-600 text-lg no-underline hover:no-underline">Notes</AccordionTrigger>
          ...
          <AccordionItem value="item-3">
            <AccordionTrigger className="font-bold text-gray-600 text-lg no-underline hover:no-underline">Video Lectures</AccordionTrigger>
            <AccordionContent className="border p-4">
              <div>
      ...
      <TableBody>
        {questions.map((ques: QuestionType, index: number) => (
          <TableRow key={index} className="cursor-pointer">
            ...
            <TableCell>
              <div className="text-xs p-1 bg-gray-100 rounded-full w-fit">{ques.createdBy.name}</div>
            </TableCell>
...
const UploadVideoFile = ({fetchData}: {fetchData: ()=>void}) => {
  ...
  const handleUpload = async () => {
    if (!fileData || !videoDesc || !videoName) return alert('File and video details are required');

    try{
      setLoading(true)
      const res = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/aws/generate-presigned-url`, {
      ...
      if (thumbnailData) {
        const thumbRes = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/aws/generate-presigned-url`, {
    ...
    // Save key and topic in the backend
      await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/video/upload/${subTopicId}`,
        {
          name: videoName,
  ...
  return <Dialog>
    ...
    <DialogContent>
      ...
      <div className='flex flex-col gap-2'>
        <Input value={videoName} onChange={(e) => { setVideoName(e.target.value) }} type="text" placeholder="Video Name"></Input>
        <Textarea value={videoDesc} onChange={(e) => { setVideoDesc(e.target.value) }} rows={2} placeholder="Video Description"></Textarea>
...
const VideoList = ({ videos, fetchData }: { videos: VideoType[], fetchData: ()=>void }) => {
      ...
      <TableBody>
        {videos.map((video: VideoType, index: number) => (
          <TableRow key={index} className="cursor-pointer">
            ...
            <TableCell>
              <div className="text-xs p-1 bg-gray-100 rounded-full w-fit">{video.createdBy.name}</div>
            </TableCell>
            <TableCell>
              <div className="flex flex-row gap-2 items-center">
                ...
                <div className="hover:scale-105 transition-all duration-300">
                  <DeleteVideoModalButton fetchData={fetchData} videoId={video._id} vidKey={video.key}/>
                </div>
              </div>
            </TableCell>
          </TableRow>
...
const UploadDocFile = ({fetchData}: {fetchData: ()=>void}) => {
  ...
  const handleUpload = async () => {
    if (!fileData || !docDesc || !docName) return alert('File and document details are required');
    ...
    // Save key and topic in the backend
      await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/docs/upload/${subTopicId}`,
        {
          name: docName,
          description: docDesc,
          createdBy: user.id,
  ...
  return <Dialog>
    ...
    <DialogContent>
      ...
      <div className='flex flex-col gap-2'>
        <Input value={docName} onChange={(e) => { setDocName(e.target.value) }} type="text" placeholder="Doc Name"></Input>
        <Textarea value={docDesc} onChange={(e) => { setDocDesc(e.target.value) }} rows={2} placeholder="Doc Description"></Textarea>
...
const DocsList = ({ docs, fetchData }: { docs: DocType[], fetchData: ()=>void }) => {
  const [show, setShow] = useState(false)
  return (
    <Table className="w-full mt-4">
      <TableHeader className="bg-gray-100">
      ...
      <TableBody>
        {docs.map((doc: DocType, index: number) => (
          <TableRow key={index} className="cursor-pointer">
            ...
            <TableCell>
              <div className="text-xs p-1 bg-gray-100 rounded-full w-fit">{doc.createdBy.name}</div>
            </TableCell>
            <TableCell>
              <div className="flex flex-row gap-2 items-center">
                <div 
                ...
                <div className="hover:scale-105 transition-all duration-300">
                  <DeleteDocModalButton docId={doc._id} docKey={doc.key} fetchData={fetchData}/>
                </div>
              </div>
            </TableCell>
          </TableRow>
...
const DeleteVideoModalButton = ({ vidKey, videoId, fetchData }: { vidKey: string, videoId: string, fetchData: () => void }) => {
  ...
  const deleteHandler = async () => {
    try {
      setLoading(true)
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/aws/delete-resource/${vidKey}`, {
        withCredentials: true
      })
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/video/${subTopicId}/${videoId}`, {
        withCredentials: true
      })
...
const DeleteDocModalButton = ({ docKey, docId, fetchData }: { docKey: string, docId: string, fetchData: () => void }) => {
  ...
  const deleteHandler = async () => {
    try {
      setLoading(true)
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/aws/delete-resource/${docKey}`, {
        withCredentials: true
      })
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/subTopic/docs/delete/${subTopicId}/${docId}`, {
        withCredentials: true
      })
...
const VideoPreview = ({url, show, setShow}: {url: string, show: boolean, setShow: (x:boolean)=>void}) => {

  return <div
  className={cn("hidden bg-transparent", {"block": show})}
  >
    <iframe
      src={url}
    />
