...
const Subject = () => {
  ...
  useEffect(() => {
    const filteredSubjects = subjects.filter(
      topic => topic.name.toLowerCase().includes(search.toLowerCase())
    )
...
const SubjectList = ({ subjects, fetchTopics }: { subjects: SubjectType[], fetchTopics: () => void }) => {
        ...
        {subjects.map((sub: SubjectType, index: number) => (
          <TableRow key={index} className='cursor-pointer hover:bg-gray-50'>
            ...
            <TableCell className='flex gap-2'>
              ...
              <EditSubject year={year as string} fetchTopics={fetchTopics} sub={sub} />
              <DeleteModalButton subId={sub._id} subName={sub.name} fetchTopics={fetchTopics} />
...
const AddSubject = ({ fetchTopics, year }: { fetchTopics: () => void, year: string }) => {
  ...
  const addSubjectHandler = async () => {
    try {
      setLoading(true)
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/subject`, 
        {
          name: subName,
          description: subDesc,
          year: Number(year)
    ...
    <Dialog>
      ...
      <DialogContent>
        ...
        <div className='flex flex-col gap-2'>
          <Input value={subName} onChange={(e) => { setSubName(e.target.value) }} type="text" placeholder="Subject Name" />
          <Textarea value={subDesc} onChange={(e) => { setSubDesc(e.target.value) }} rows={2} placeholder="Subject Description (optional)" />
        </div>
        <DialogFooter>
          <Button type='submit' onClick={addSubjectHandler}
...
const EditSubject = ({ fetchTopics, year, sub }: { fetchTopics: () => void, year: string, sub: SubjectType }) => {
  ...
  const updateSubjectHandler = async () => {
    try {
      setLoading(true)
      const response = await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/subject/${sub._id}`, 
        {
          name: subName,
          description: subDesc,
          year: Number(year)
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <div className='flex flex-col gap-2'>
          <Input defaultValue={subName} onChange={(e) => { setSubName(e.target.value) }} type="text" placeholder="Subject Name" />
          <Textarea defaultValue={subDesc} onChange={(e) => { setSubDesc(e.target.value) }} rows={2} placeholder="Subject Description (optional)" />
        </div>
        <DialogFooter>
          <Button type='submit' onClick={updateSubjectHandler}
          className='bg-gradient-to-b from-gray-600 to-gray-900 rounded-md shadow-sm p-2 px-4 text-sm hover:scale-105 duration-300 transition-all'
          >
            {
    ...
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This action cannot be undone. It will permanently delete this subject.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
